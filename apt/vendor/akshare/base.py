...
class base():
    ...
    def __init__(self , rds_host = None , myauth = True):           
        ...
        if rds_host is None:    #数据源为空时进行映射
            ...
            "ubuntu191": self.数据源.ubuntu191, #以上为老接口，可能不适用
            "UBUNTU186_DB_CONN": self.数据源.ubuntu186, #以上为新接口，配合env文件使用
            "AWS_DB_CONN": self.数据源.aws,  # Add this mapping for your env variable
            }      
...
#    __tstype = 'D'
    ...
    def __init__(self , code = None , start_date = datetime(2021,1,1), end_date = datetime.now() , ktype = "1d" , fq = 复权.动态复权  ):
        ...
        self.fq = fq
        self.api = None
        self.dict = {'1d':'D','1m':'1','5m':'5','30m':'30min','60m':'60'}
        #self.myauth = myauth
        #self.server = fwq
        #数据校验环节：
        #1. 证券代码不能为空
