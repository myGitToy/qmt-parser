...
- 提供 `qmtApi` 对象，包含以下方法：
  ...
  - `getSectorCategories()` - 获取板块分类类别列表
  - `getSectorFiles(category, page, pageSize)` - 获取板块文件列表

**状态管理** (`src/stores/qmtStore.ts`)
- 使用 Zustand 管理全局状态
- 存储数据：市场列表、周期列表、财务类型、Tick股票列表、ETF列表、板块分类列表、文件列表、分页信息
- 核心状态：`activeDataType` 控制当前展示的数据类型（kline/finance/dividend/tick/etf/sector）
- 提供操作方法：数据类别切换、各类型数据获取、选择、文件查看

**UI 组件** (`src/pages/qmt/QmtDataExplorer.tsx`)
- 主页面组件，采用金融终端风格布局
- 功能区切换：`<Segmented>` 胶囊式控件，6种数据类型
...
- **核心类**：`QmtDataService`
- **数据来源**：QMT 客户端本地 `datadir` 目录及相邻目录（`finance/`、`DividData/`、`Sector/`）
...
```typescript
// 数据类别（控制UI功能区切换）
type DataType = "kline" | "finance" | "dividend" | "tick" | "etf" | "sector";
...
datadir/
└── Sector/                          # 板块分类根目录
    ├── 申万一级行业板块/             # 申万一级行业 (37个板块)
    │   ├── SW1银行                  # 板块文件，内容为逗号分隔的股票代码
    │   ├── SW1房地产
    │   └── sectorConfig.xml         # 元数据配置（不展示）
    ├── 申万二级行业板块/             # (182个板块)
    ├── 申万三级行业板块/             # (454个板块)
    ├── 证监会行业板块/               # (19个板块)
    ├── 证监会二级行业板块/           # (89个板块)
...
| 端点 | 方法 | 描述 | 参数验证 |
|------|------|------|---------|
| `/api/v1/qmt/sector/categories` | GET | 列出板块分类类别 | - |
| `/api/v1/qmt/sector/categories/{category}/files` | GET | 列出板块文件（分页） | `category` 路径遍历校验 |
