...
def convert_ts_model(inst):
    ...
    if 'MIN' in inst_data['marketDataType']:
        ts_inst = convert_min_model(datatype_map1[inst.marketDataType], inst_data['mdKLine'])
...
def convert_min_model(freq, md_kline) -> TsMin:
    ds = str(md_kline['MDDate'])
    ts = str(md_kline['MDTime'])
    inst: TsMin = TsMin(
        ts_code=md_kline['HTSCSecurityID'],
        freq=freq,
        trade_time=f'{ds[:4]}-{ds[4:6]}-{ds[6:]} {ts[:-7]}:{ts[-7:-5]}',
        open=md_kline['OpenPx'],
        close=md_kline['ClosePx'],
        high=md_kline['HighPx'],
        low=md_kline['LowPx'],
        vol=md_kline['TotalVolumeTrade'],
        amount=md_kline['TotalValueTrade'],
        open_int=md_kline['KLineCategory']
    )
    return inst
...
def convert_tick_stock(md_stock) -> TsTick:
    ...
    inst: TsTick = TsTick(
        ts_code=md_stock['HTSCSecurityID'],
        name='',
...
def convert_tick_index(md_index) -> TsTickIdx:
    ...
    inst: TsTickIdx = TsTickIdx(
        ts_code=md_index['HTSCSecurityID'],
        name='',
        trade_time=f'{ds[:4]}-{ds[4:6]}-{ds[6:]} {ts[:-7]}:{ts[-7:-5]}:{ts[-5:-3]}',
        pre_price=md_index.get('PreClosePx'),
        price=int(md_index.get('LastPx', 0)) / 10000,
...
def convert_tick_option(md_option) -> TsTickOpt:
    ...
    inst: TsTickOpt = TsTickOpt(
        ts_code=md_option['HTSCSecurityID'],
        instrument_id='',
        trade_time=f'{ds[:4]}-{ds[4:6]}-{ds[6:]} {ts[:-7]}:{ts[-7:-5]}:{ts[-5:-3]}',
...
def convert_tick_future(md_future) -> TsTickFuture:
    ...
    inst: TsTickFuture = TsTickFuture(
        ts_code=md_future['HTSCSecurityID'],
        trade_time=f'{ds[:4]}-{ds[4:6]}-{ds[6:]} {ts[:-7]}:{ts[-7:-5]}:{ts[-5:-3]}',
