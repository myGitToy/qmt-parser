...
class ExcelReader():
    ...
    def read_excel(self):
        try:
            ...
            df = df.drop([0], axis=0)
            #加载Time和VRTG列
            multi_columns = [self.parameters['着陆载荷']['英文名']] + [f"{self.parameters['着陆载荷']['英文名']}.{i}" for i in range(1, 16)]
            final_columns = [self.parameters['时间']['英文名']] + multi_columns
            df = df[final_columns]
            # Time进行时间转换并且变成索引
            #df[self.parameters['时间']['英文名']] = pd.to_datetime(df[self.parameters['时间']['英文名']])
            df = df.set_index(self.parameters['时间']['英文名'])
            # 将多列数据转换为长格式
            df_long = df.reset_index().melt(id_vars=[self.parameters['时间']['英文名']], value_vars=multi_columns, var_name='VRTG', value_name='Value')
            #按照Time VRTG排序，升序
            df_long = df_long.sort_values(by=[self.parameters['时间']['英文名'], 'VRTG'])
            print(df_long)
            # 生成新的时间索引，间隔为1/16秒
            num_rows = len(df_long)
            start_time = pd.to_datetime(df_long[self.parameters['时间']['英文名']].iloc[0].strftime('%H:%M:%S'))
            end_time = start_time + pd.to_timedelta((num_rows - 1) / 16, unit='s')
            new_time_index = pd.date_range(start=start_time, end=end_time, periods=num_rows)
            
            # 将新的时间索引添加到数据框中
            df_long['Time'] = new_time_index
            
            # 重新排序列
            df_long = df_long[['Time', 'VRTG', 'Value']] 
            print(df_long)     
            
            
            
            
            #将df中包含{multi_columns}的列转行，列名重命名为VRTG，Time索引转换成1/16秒的时间序列
            df = df.stack().reset_index()
            print(df)
            df.columns = ['Time', 'VRTG', 'Value']
            df['Time'] = pd.to_datetime(df['Time']) + pd.to_timedelta(df.groupby(df.index // 16).cumcount() * (1/16), unit='s')
