...
from apt.vendor.jqdata.security import security as security
...
class TestStrategy(bt.Strategy):
    params=(('high_period',25),     #最高价的计算周期（日线默认25 小时线默认100）
            ...
            ('prank_period',40),    #分位数的计算周期（日线默认25 小时线默认100）
            ('R',0.5),             #风险值R设定
    ...
    def notify_order(self, order):
        ...
        if order.status in [order.Completed]:
            if order.isbuy():
                #self.log('BUY EXECUTED, Price: %.3f, Cost: %.3f, Comm %.2f' % (order.executed.price,order.executed.value,order.executed.comm))
...
if __name__ == '__main__':
    ...
    #code_list1 = jqdata.read_excel(file_name = '.\\data\\海龟模型\\自选股列表.xlsx' , sheet_name = '33指数')['证券代码'].tolist()
    #code_list2 = jqdata.read_excel(file_name = '.\\data\\海龟模型\\自选股列表.xlsx' , sheet_name = 'ETF')['证券代码'].tolist()
    code_list1 = jqdata.read_excel(file_name = '.\\data\\海龟模型\\backtrader测试股列表.xlsx' , sheet_name = 'ETF')['CODE'].tolist()
    #code_list2 = jqdata.read_excel(file_name = '.\\data\\海龟模型\\backtrader测试股列表.xlsx' , sheet_name = 'CAP200')['CODE'].tolist()
    #######2. 合并自选股列表并去重
    ...
    #,
    n = 0
    df = pd.DataFrame(columns = ['证券代码','证券名称','证券类型','最高价周期','ATR周期','分位数周期','R基准值','ATR基准','头寸基准','清仓ATR基准','清仓日线收盘价基准','期初价格','期末价格','基金期末价值','价格涨跌幅','净值涨跌幅','最大回撤','夏普比率'])
    for code in code_list:
        n += 1
        print(f'正在处理{n}/{len(code_list)}，证券代码：{code}')
        start = datetime.datetime(2016,10,19)
        end = datetime.datetime(2021,12,31)
        ktype = '1d'
        sec = security(rds_host = jqdata.数据源.localhost , myauth= False)
        ...
        #cerebro.optstrategy(TestStrategy,'high_period'=)
        #增加最大回撤观察窗口
        ...
        #print('夏普比率:', thestrat.analyzers.mysharpe.get_analysis()['sharperatio'])
        thestrat.analyzers.DW.get_analysis()
        #cerebro.plot()
        #使用增强型图形输出
        ...
        #获取区间收盘信息
        close_dict = d.get_close_range( code = code , start_date = start , end_date = end , ktype = ktype )
        #获取基础信息
        df_sec = sec.get_security(code = code , day = end)
        name = df_sec.iloc[0].at['display_name']
        type = df_sec.iloc[0].at['type']
        #回测的参数信息
        dict_cerebo = {}

        #增加到DataFrame
        df = df.append([{'证券代码' : code , '证券名称' : name , '证券类型' : type ,
                         
                         
                         '基金期末价值' : cerebro.broker.getvalue() , '期初价格' : close_dict['beginning_close'] , '期末价格' : close_dict['ending_close'] , '净值涨跌幅' : (cerebro.broker.getvalue() - 1000000)/1000000 ,'价格涨跌幅' : ((close_dict['ending_close'] - close_dict['beginning_close'])/close_dict['beginning_close']) , 
                         "夏普比率" : thestrat.analyzers.mysharpe.get_analysis()['sharperatio'] , "最大回撤" : thestrat.analyzers.mydrawdown.get_analysis()['max']['drawdown']}] , 
                        ignore_index=True)
    df.to_excel(f'.\\data\\测试数据\\backtrader测试.xlsx', sheet_name = f'sheet1' ,  header=True, index=False)
    print(df)