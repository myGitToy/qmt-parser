...
2. 从tspro中读取更新日期起止时间段的数据
3. 数据去重处理后重新写入H5文件
4. 数据保存格式：code和date作为index写入H5文件
备注：
...
df_tspro = pd.DataFrame() #ts_pro主数据
a = data()
nw = datetime.now()
...
for dt in rrule.rrule(rrule.MONTHLY, dtstart = a.start_date, until = a.end_date):    
